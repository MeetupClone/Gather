...
export class GroupDashboard extends Component {
    ...
    render() {
    	const { editGroup, deleteGroup } = this.props
        ...
        if (this.state.groupOrganizerUid !== this.state.currentUserUid) {
            notAuthenticated = (<h1> You can't edit this group, you aren't the group owner. </h1>)
        } else {
            deleteConfirm = (<button onClick={() => this.setState({deleteConfirm: true})}>Delete group</button>)
        }

        if (this.state.groupName === this.state.confirmDeleteInput) {

            finalDeleteButton = (<div><br/><button onClick={() => deleteGroup(this.state)}> Delete group </button></div>)
        }
...
{notAuthenticated}
              {deleteConfirm}
				...
				<input placeholder={this.state.groupName} onChange={(event) => {this.setState({groupName:event.target.data})}} />
				<br/>
				<img src={this.state.imagePreviewUrl || this.state.groupPic} alt={this.state.groupName}></img>
                 <input className="fileInput" 
                    type="file" 
                    onChange={(event)=>this.submitImageUpload(event)} />
                <br/>
                Change Your Group Description
                <textarea type="text" id="noter-text-area" name="editableDescription" value={this.state.groupDesc} onChange={this.handleChange}></textarea>
			</div>
